...
class ResetPasswordController extends Controller
{
    ...
    */
    ...
    {
        // $cekToken =  DB::table('users')->where('token_reset','=',$token)->count();
        $cekToken = User::where('token_reset','=',$token)->count();
        if($cekToken > 0){
            //Bisa ke view reset
            // $cekDate =  DB::table('users')->where('token_reset','=',$token)->first();
        ...
        return view('masterweb::auth.passwords.reset', ['token' => $token, 'status'=>$status]);
    }
    ...
    public function ResetPasswordEmail(Request $request)
    {
        ...
        $updateToken = User::where('email',$email)->update($update);
        
        $detailMail = [
            'title' => 'Informasi MasterWeb',
            'nama' => $cekMail->name,
    ...
    public function ResetPasswordCustome(Request $request)
    {
        ...
        // $set = DB::table('users')->where('token_reset','=',$token)->update(['password'=>Hash::make($request->post('password'))]);
