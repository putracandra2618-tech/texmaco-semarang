...
class AdmMenuController extends Controller
{
  ...
  {
    $parent = AdminMenu::all()->where('upmenu', null);
    $data = AdminMenu::all()->sortBy('order')->where('publish', '1')->where('upmenu', '0');
    return view('masterweb::module.admin.menu.list', compact('data', 'parent'));
  }
  ...
  public function store(Request $request)
  {
    ...
    return redirect(url('menuadm'))->with('status', 'Data berhasil disimpan!');
  }
