...
$get_layanan = \Smt\Masterweb\Models\Layanan::where('menu_id', $menu_id)->orderby('urutan', "ASC")->get();

if(!empty($cat))
{
    $getService = \Smt\Masterweb\Models\Layanan::where('link_url', $cat)->first();
}else{
    $getService = \Smt\Masterweb\Models\Layanan::orderby('urutan', "ASC")->where('menu_id', $menu_id)->first();
}
// if (empty($sesi)) {
