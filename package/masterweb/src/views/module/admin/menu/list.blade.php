        ...
        @foreach ($data as $menu)
          <div class="main-menu-container">
            <div class="card rounded border mb-2 main-menu" id="{{ $menu->id }}">
                      ...
                      <div class="p-2 bd-highlight">
                        <p>
                          ...
                          <form onsubmit="return confirm('Delete this menu permanently?')" class="d-inline"
                            action="{{ url('menuadm/destroy/' . $menu->id) }}" method="POST">
                            @csrf
              ...
              @php
                $subdata = \Smt\Masterweb\Models\AdminMenu::all()
                    ->where('publish', '1')
                    ->sortBy('order')
                            ...
                            <div class="p-2 bd-highlight">
                              <p>
                                ...
                                <form onsubmit="return confirm('Delete this user permanently?')" class="d-inline"
                                  action="{{ url('menuadm/destroy/' . $submenu->id) }}" method="POST">

                                  @csrf

                                  <input type="hidden" name="_method" value="DELETE">
                                  @if(SmtHelp::getAction("update"))
                                  <button type="button" class="btn btn-primary btn-sm edit-module"
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="post" action="{{ url('menuadm/update') }}">
            ...
            <label>Type</label>
            <input type="radio" name="type_link" id="type_link" value="0">Link | <input type="radio" name="type_link" id="type_url"
              value="1">Url
        </div>
        <div class="modal-footer">
          <button type="submit" class="btn btn-success">Simpan</button>
...
@section('scripts')
  <script>
    var set_containers = $('.main-menu-container').toArray();
