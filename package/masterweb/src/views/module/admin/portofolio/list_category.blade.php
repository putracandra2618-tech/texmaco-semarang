      ...
      <div class="d-flex">
        ...
        <div class="p-2">
          @if(SmtHelp::getAction("create"))
            <a href="{{ route('adm-categoryportofolio.create') }}">
            ...
            <table id="order-listing" class="table">
              ...
              <tbody>
                ...
                @foreach ($data as $data)
                  <tr>
                    <td>{{ $no++ }}</td>
                    <td> {{ ucfirst($data->name_category_portofolio) }} </td>
                    @if((SmtHelp::getAction("update")||SmtHelp::getAction("delete")))
                    <td>
                      @if(SmtHelp::getAction("update"))
                      <a href="{{ route('adm-categoryportofolio.edit', [$data->id_category_portofolio]) }}">
                      ...
                      <form onsubmit="return confirm('Apakah anda yakin ingin menghapus data ini?')"
                        class="d-inline"
                        action="{{ route('adm-categoryportofolio.destroy', [$data->id_category_portofolio]) }}"
                        method="POST">
