@php
  ...
  $privilege = \App\Models\Privileges::where('id',$user->level)->first();
@endphp
<nav class="sidebar sidebar-offcanvas" id="sidebar">
  <ul class="nav">
    ...
    {{-- LIST MENU --}}
    @php
        $parent = \App\Models\AdminMenu::all()->sortBy('order')->where('upmenu','0');
    @endphp
    @foreach ($parent as $menu)
      @php
      $role = \App\Models\Role::where('menu_id',$menu->id)->where('privilege_id',$privilege->id)->first();
      ...
      $child = \App\Models\AdminMenu::all()->sortBy('order')->where('upmenu',$menu->id);
      @endphp
     <li class="nav-item">
      @if (count($child) > 0)
        ...
        <div class="collapse" id="menu-{{SmtHelp::create_link($menu->name)}}">
            <ul class="nav flex-column sub-menu"> 
              ...
              @php
                $role = \App\Models\Role::where('menu_id',$submenu->id)->where('privilege_id',$privilege->id)->first();
