              ...
              <div class="input-group col-md-4">
                ...
                <select name="action" id="action" onchange="submit()" class="form-control">
                  ...
                  @foreach ($actions as $action)
                    <option value="{{ $action }}" {{ SmtHelp::isSelected($action, request()->action) }}>
                      {{ $action }}</option>
                  @endforeach
                </select>
              </div>
          ...
          @foreach ($module as $mdl)
            <div class="card rounded border mb-2 grabbable">
              <div class="card-body p-3 moduleId" data-id="{{ $mdl->id }}">
                <div class="media">
                  <button class="btn btn-config btn-default btn-sm text-primary align-self-center mr-3"
                    data-id="{{ $mdl->id }}" type="button"><i class="fa fa-cog icon-sm"></i></button>
            ...
            <div class="card-body p-3">
              <div class="media">
                ...
                <div class="media-body">
                  ...
                  <p class="mb-0 text-muted">
                    <textarea name="html_code" class="form-control moduleId" data-id="{{ 'html' }}" id="" cols="30"
                      rows="3"></textarea>
                  </p>
                </div>
              </div>
