...
@php
    ...
    if (empty($sesi)) {
        $data = \App\Models\Portofolio::where('publish','1')->orderBy('created_at','DESC')->paginate(9);
    } else {
        $data = \App\Models\Portofolio::where('publish','1')->whereHas('category_portofolio',function ($qw) use ($cat)
        {
            $qw->where('link_category_portofolio',$cat);
        })->orderBy('created_at','DESC')->paginate(9);
    }

    $data_cat = \App\Models\CategoryPortofolio::orderBy('created_at', 'ASC')->get();
